...
#############################################################################这里做了修改，将参数列表的参数改成了一个
# eval_metrics = ['silog', 'abs_rel', 'log10', 'rms', 'sq_rel', 'log_rms', 'd1', 'd2', 'd3']

eval_metrics = [ 'abs_rel']
...
def compute_errors(gt, pred):
    ...
    # log10 = np.mean(err)
    silog=abs_rel=log10=rms=sq_rel=log_rms=d1= d2=d3=1
    return [silog, abs_rel, log10, rms, sq_rel, log_rms, d1, d2, d3]
####################################################
def compute_psnr(gt, pred,max_value=255.0):
    # # 将CUDA tensor移动到CPU上
    # gt_cpu = gt.cpu()
    # pred_cpu = pred.cpu()

    # # 将CPU tensor转换为NumPy数组
    # gt_numpy = gt_cpu.numpy()
    # pred_numpy = pred_cpu.numpy()

    gt = gt*255.0
    gt_uint8 = np.uint8(gt)
    pred = pred*255.0
    pred_uint8 = np.uint8(pred)
    mse = np.mean((gt_uint8 - pred_uint8) ** 2)
...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        ...
        depth_gt = depth_gt / 3.0
        # loss = nn.MSELoss()
        loss = nn.L1Loss()
        return loss(depth_est,depth_gt)
