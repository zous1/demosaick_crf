...
##############################v-dim=[512->256->128->64]
    def __init__(self,
        ...
        self.ffn = FeedForward(embed_dim, 2, bias=True)
        
    def forward(self, x, v):

#################################################v是上一层传下来的输出，x是编码器的输出
...
####################################门控机制，
        res = x
        res_1 = res.permute(0, 2, 3, 1).contiguous()
        res_1 = res_1.view(-1, Wh * Ww, self.embed_dim)
        res_1 = self.ffn(self.norm(res_1)) 
        res_1 = res_1.permute(0, 2, 1).contiguous()
        res_1 = res_1.view(-1, self.embed_dim, Wh , Ww)+ res
###################Wh,Ww =(15 20),30 40,60 80, 120 160
