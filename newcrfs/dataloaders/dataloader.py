...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            #######################################################
            # depth_gt_array = np.array(image) # 把图片转换成数组
            # np.set_printoptions(threshold=np.inf)
            # print(image.size) # 打印数组
            ...
            #将原先的1000.0改成了255.0，对读取的深度gt做了处理
...
################################################数据增强删掉0508
        ...
        ############################查看sample里面image的值，已经由于标准化的处理而改变
    ...
    #     # brightness augmentation亮度增强的部分
    #     if self.args.dataset == 'nyu':
    #         brightness = random.uniform(0.75, 1.25)
    #     else:
    #         brightness = random.uniform(0.9, 1.1)
    #     #将图像数据image_aug与亮度系数brightness相乘，以实现图像的亮度增强
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'train':
            ...
            depth = depth * 255.0  # 将深度图像像素值乘以255
