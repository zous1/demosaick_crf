...
from utils import post_process_depth, flip_lr, silog_loss, compute_errors, compute_psnr,eval_metrics, \
...
# parser.add_argument('--pretrain',       nargs=2  , type=str,   help='path of pretrained encoder', default=None)
...
# 配置日志记录器
logging.basicConfig(filename='psnr.log', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')

def online_eval(model, dataloader_eval, gpu, ngpus, epoch,post_process=False):
    eval_measures = torch.zeros(10).cuda(device=gpu)
    #################################?
    psnr=torch.zeros(1).cuda(device=gpu)
    sum_psnr=0
    sum_w_psnr=0
...
####################################################旨在处理深度图像以改善其质量
        ...
        if args.garg_crop or args.eigen_crop:
            ...
            gt_height, gt_width = gt_depth.shape[0], gt_depth.shape[1]
        ...
        pred_depth = np.transpose(pred_depth, (1, 2, 0))
        #################################这里把mask去掉了
        # measures = compute_errors(gt_depth[valid_mask], pred_depth[valid_mask])
        measures = compute_errors(gt_depth, pred_depth)
        ###########################################添加一个psnr的计算   
        psnr= compute_psnr(gt_depth, pred_depth*3.0)
        sum_psnr          += torch.tensor(psnr).cuda(device=gpu)
    ...
    if args.multiprocessing_distributed:
        ...
        dist.all_reduce(tensor=sum_psnr,op=dist.ReduceOp.SUM, group=group)

    if not args.multiprocessing_distributed or gpu == 0:
        eval_measures_cpu = eval_measures.cpu()
        sum_psnr_cpu=sum_psnr.cpu()
        cnt = eval_measures_cpu[9].item()
        eval_measures_cpu /= cnt
        # print('cnt:',cnt*ngpus)
        ######################################
        print('psnr:',sum_psnr_cpu/(cnt*ngpus))
        print('psnr of w:',sum_w_psnr/(cnt*ngpus))
        # 将结果写入日志文件
        logging.info(f'epoch: {epoch} psnr: {sum_psnr/(cnt*ngpus)}')
        # logging.info(f'psnr: {sum_w_psnr/56.0}')
...
################################################################加了
    ...
    while epoch < args.num_epochs:
        ...
        with tqdm(total=steps_per_epoch) as tq:
...
##################################################################
        ...
        ####################################先不测试了，先保存模型吧，把下面四行注释掉
                    time.sleep(0.1)
                    model.eval()
                    with torch.no_grad():
                        eval_measures = online_eval(model, dataloader_eval, gpu, ngpus_per_node,epoch, post_process=True)
                    eval_measures = 1
                ...
                model_just_loaded = False
                global_step += 1
                tq.set_postfix({"[epoch][s/s_per_e/gs]": "[{}][{}/{}/{}], loss: {:.12f}".format(epoch, step, steps_per_epoch, global_step, loss)})
                tq.update(1)
        epoch += 1
